...
**Native Linux GUI for ISDT C4 Air chargers** – no Home Assistant, no cloud, direct Bluetooth Low Energy (BLE) connection.  
Now with **full control** over charging parameters!
...
### 📊 Monitoring
- **BLE Connection** – Direct communication with ISDT C4 Air (and compatible devices)
...
| ISDT NP2 Air| ⚠️ Theoretically compatible (read on```markdown
```
...
```bash
git clone https://github.com/DittelHome/ISDT-C4-Air-Control.git
cd ISDT-C4-Air-Control
```
...
### Cut‑off field is disabled
For `Auto` and `LiIon(1.5V)`, cut‑off is not supported – the field is automatically disabled.
...
The BLE protocol is based on the documentation from the [Home Assistant Integration](https://github.com/mtheli/isdt_air_ble) for reading, and reverse‑engineered from the ISD Link Android app for writing.

### Adding New Battery Types

To add a new battery type, edit `isdt_limits.py` and add an entry to the `BATTERY_LIMITS` dictionary. Also update the dropdown list in `isdt.py` (battery_combo values).
